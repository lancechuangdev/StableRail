...
func run() error {
	...
	paymentWebhookLoop := &consumer.Loop{
		Reader:    consumer.NewKafkaReader(config.KafkaBrokers, string(eventbus.PaymentEventsTopic), "stablerail-payment-webhooks"),
	...
	payins, err := payin.NewService(db, provider, provider)
	if err != nil {
		return err
	}
