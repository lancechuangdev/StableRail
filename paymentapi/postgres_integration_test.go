...
func TestPostgresPaymentHTTPIntegration(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, server.URL+"/v1/payments", strings.NewReader(`{"direction":"payout","external_reference":"e2e-order","currency":"USD","amount_minor":4200,"tenant_id":"e2e-tenant"}`))
