...
type Config struct {
	...
	CircleMinRequestInterval                                                                     time.Duration
	KafkaBrokers                                                                                 []string
...
func ConfigFromEnv() (Config, error) {
	c := Config{
		...
		CircleMinRequestInterval: time.Second / 5,
	...
	if raw := os.Getenv("STABLERAIL_CIRCLE_MIN_REQUEST_INTERVAL"); raw != "" {
		d, err := time.ParseDuration(raw)
		if err != nil || d < 0 {
			return Config{}, errors.New("invalid STABLERAIL_CIRCLE_MIN_REQUEST_INTERVAL")
		}
		c.CircleMinRequestInterval = d
	}
