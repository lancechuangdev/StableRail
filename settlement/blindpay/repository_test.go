...
func TestGetApprovedPayoutProfile(t *testing.T) {
	...
	defer db.Close()
	repo, _ := NewRepository(db)
	columns := []string{"tenant_id", "provider_customer_id", "kyc_status", "provider_bank_account_id", "rail", "display_name", "account_last_four", "status", "provider_wallet_id", "network", "address", "display_name", "status"}
	mock.ExpectQuery("SELECT c.tenant_id").WithArgs("cus_local", "ba_remote", "bl_remote").WillReturnRows(sqlmock.NewRows(columns).AddRow("cus_local", "re_remote", "approved", "ba_remote", "ach", "Payroll", "1234", "approved", "bl_remote", "sepolia", "0xabc", "Settlement", "active"))
...
func TestGetPayoutProfileRejectsUnapprovedCustomer(t *testing.T) {
	...
	repo, _ := NewRepository(db)
	columns := []string{"tenant_id", "provider_customer_id", "kyc_status", "provider_bank_account_id", "rail", "display_name", "account_last_four", "status", "provider_wallet_id", "network", "address", "display_name", "status"}
	mock.ExpectQuery("SELECT c.tenant_id").WillReturnRows(sqlmock.NewRows(columns).AddRow("cus_local", "re_remote", "verifying", "ba_remote", "ach", "Payroll", "1234", "approved", "bl_remote", "sepolia", "0xabc", "Settlement", "active"))
...
func TestReferenceValidation(t *testing.T) {
	if err := (CustomerReference{TenantID: "x", ProviderCustomerID: "wrong", KYCStatus: "approved"}).Validate(); err == nil {
		t.Fatal("invalid provider customer ID accepted")
	}
	if err := (BankAccountReference{ProviderBankAccountID: "ba_x", TenantID: "x", Rail: "ach", DisplayName: "x", AccountLastFour: "12345", Status: "approved"}).Validate(); err == nil {
