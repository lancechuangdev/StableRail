...
func (s *PayoutService) SubmitPayment(ctx context.Context, paymentID, idempotencyKey string) (*PayoutSubmission, error) {
	...
	now := s.now()
	_, err = s.db.ExecContext(ctx, `UPDATE payouts SET provider_payout_id=$1,provider_status=$2,provider_payload=$3,last_error=NULL,updated_at=$4,submitted_at=$4 WHERE payment_id=$5 AND provider_status IN ('submission_pending','unknown')`, payout.ID, payout.Status, raw, now, paymentID)
	if err != nil {
...
func (s *PayoutService) RecoverUnknownOnce(ctx context.Context) (int, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT payment_id,idempotency_key FROM payouts WHERE provider_status='unknown' ORDER BY updated_at LIMIT 100`)
	if err != nil {
...
func (s *PayoutService) recordError(ctx context.Context, paymentID, status string, cause error) error {
	_, err := s.db.ExecContext(ctx, `UPDATE payouts SET provider_status=$1,last_error=$2,updated_at=$3 WHERE payment_id=$4 AND provider_status='submission_pending'`, status, cause.Error(), s.now(), paymentID)
	if err != nil {
		return err
	}
	return nil
