...
func (s *PostgresService) enqueue(ctx context.Context, tx *sql.Tx, paymentID, eventType string, payload []byte, now time.Time) error {
	workflowType := map[string]string{
		"payment.created":    "payout.created",
		"payment.processing": "payout.submitted",
		"payment.succeeded":  "payout.completed",
	}[eventType]
	if workflowType == "" {
	...
	if _, err = tx.ExecContext(ctx, `
		...
		workflowID, eventbus.PayoutEventsTopic, workflowType, payoutEventVersion(workflowType), paymentID, "payout", payload, now,
	); err != nil {
	...
	if _, err = tx.ExecContext(ctx, `
		INSERT INTO outbox_events
			(id, topic, event_type, event_version, aggregate_id, aggregate_type, payload, occurred_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		publicID, eventbus.PaymentEventsTopic, eventType, paymentEventVersion(eventType), paymentID, "payment", payload, now,
	); err != nil {
