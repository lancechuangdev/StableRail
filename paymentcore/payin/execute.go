...
func (s *Service) ExecutePayin(ctx context.Context, payinID string) (ExecuteResult, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT COALESCE(q.provider_quote_id,''),p.idempotency_key,p.settlement_status,p.provider_payin_id,p.tenant_id,p.funding_method,COALESCE(p.source_instrument_id,''),p.destination_account_id,p.source_amount_minor,p.source_currency,p.destination_amount_minor,p.destination_currency FROM payins p LEFT JOIN payment_quotes q ON q.id=p.quote_id WHERE p.payment_id=$1 AND p.provider=$2 FOR UPDATE`, payinID, s.executionProvider.Name()).Scan(&providerQuote, &key, &status, &providerID, &tenantID, &fundingMethod, &sourceInstrumentID, &destinationAccountID, &sourceAmountMinor, &sourceCurrency, &destinationAmountMinor, &destinationCurrency)
	...
	if providerID.Valid && providerID.String != "" {
		...
		if err := tx.QueryRowContext(ctx, `SELECT instructions,provider_payload FROM payins WHERE payment_id=$1`, payinID).Scan(&instructions, &payload); err != nil {
			return ExecuteResult{}, err
		}
	...
	if _, err := tx.ExecContext(ctx, `UPDATE payins SET settlement_status='submission_pending',updated_at=$1 WHERE payment_id=$2`, s.now(), payinID); err != nil {
		return ExecuteResult{}, err
	}
	if err := tx.Commit(); err != nil {
		return ExecuteResult{}, err
