...
func TestRecordLedgerCompletesReceivedPayin(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery("SELECT tenant_id,status,destination_amount_minor").WithArgs("pin_1").WillReturnRows(sqlmock.NewRows([]string{"tenant_id", "status", "amount", "currency"}).AddRow("tenant_1", "received", int64(9900), "USDC"))
	mock.ExpectExec("INSERT INTO ledger_transactions").WithArgs("jrn_pin_1_succeeded", "pin_1", now).WillReturnResult(sqlmock.NewResult(0, 1))
	...
	mock.ExpectExec("INSERT INTO webhook_deliveries").WithArgs("evt_pin_1_succeeded", "pin_1", "payin.succeeded", sqlmock.AnyArg(), now, "tenant_1").WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectExec("INSERT INTO outbox_events").WithArgs("evt_pin_1_succeeded", EventsTopic, "payin.succeeded", 1, "pin_1", sqlmock.AnyArg(), now).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
