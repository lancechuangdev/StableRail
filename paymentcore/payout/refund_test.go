...
func TestCreateRefundCreatesLinkedPaymentAndPaymentCreatedEvent(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO outbox_events").WithArgs("evt_test", eventbus.PaymentEventsTopic, "payment.created", 1, "pay_test", "payment", sqlmock.AnyArg(), now).WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectCommit()
